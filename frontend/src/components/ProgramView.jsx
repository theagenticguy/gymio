  ...
  return (
    ...
    >
      ...
      <div className="flex gap-6">
        <div className="space-y-2">
          <label className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground/50">
            Days / Week
          </label>
        ...
        <div className="space-y-2">
          <label className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground/50">Weeks</label>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-xl bg-white/[0.03] border border-white/[0.06] px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-1 focus:ring-purple-500/30 resize-none"
        />
      </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground/50 mt-0.5">
            ...
            {exercise.equipment && (
              <span className="text-muted-foreground/30"> · {exercise.equipment}</span>
            )}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="text-right">
            <span className="text-sm font-bold">
              {exercise.sets}
              <span className="text-muted-foreground/40 mx-0.5">&times;</span>
              {exercise.reps}
            </span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className="text-sm font-semibold text-muted-foreground/50">{dayName}</span>
        <span className="text-[11px] text-muted-foreground/30">Rest</span>
      </div>
    </motion.div>
  );
}
...
function ProgressionRules({ rules }) {
  ...
  return (
    ...
    >
      <h4 className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground/50">
        Progression Rules
      </h4>
      <div className="space-y-2">
        {rules.map((rule, i) => (
          <div key={i} className="text-[13px] leading-relaxed">
            ...
            <span className="text-muted-foreground/60">{rule.rule}</span>
          </div>
        ))}
      </div>
    </motion.div>
        ...
        {Object.entries(targets).map(([muscle, sets]) => {
          ...
          return (
            <div key={muscle} className="flex items-center gap-1.5 text-[12px]">
              ...
              <span className="text-muted-foreground/60">{muscle}</span>
    ...
    <div className="space-y-5 max-w-2xl mx-auto">
      ...
      {program.deload_strategy && (
        ...
        >
          ...
          <span className="text-muted-foreground/60">{program.deload_strategy}</span>
        </motion.div>
      )}
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center gap-3 py-16">
        ...
        <p className="text-sm text-muted-foreground/60">Loading program...</p>
      </div>
    );
  }
